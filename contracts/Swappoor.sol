...
contract bribeSwappoor is Initializable, AccessControlEnumerableUpgradeable, UUPSUpgradeable {
    ...
    IPoolRouter constant depositor = IPoolRouter(0x1863736c768f232189F95428b5ed9A51B0eCcAe5);
    ...
    function initialize(address admin, address setter, address _proxyAdmin) external initializer {
    ...
    */
    function approveDepositor() external {
        IERC20Upgradeable(neadRamWeth).approve(address(depositor), type(uint).max);
    }
    ...
    */
    function zapIn(bool isWeth, uint amountA, address to) external {
        ...
        IERC20Upgradeable(depositor.tokenForPool(neadRamWeth)).transfer(to, liquidity);
    }
    ...
    function swapTokens(address tokenA, address tokenB, uint amount) external returns (uint) {
        IERC20Upgradeable(tokenA).transferFrom(msg.sender, address(this), amount);
